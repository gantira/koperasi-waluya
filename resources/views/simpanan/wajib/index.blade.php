                ...
                <div class="card">
                    <div class="header">
                        ...
                        <div class="clearfix">
                            ...
                            <div class="pull-left">
                                <p class="category "><a href="{{ url('wajib/create') }}"><i class="ti-layers"></i> Tambah</a></p>
                            </div>
                            <div class="form-inline pull-right">
                                {{-- {!! Form::open(['url'=>'wajib', 'method'=>'get']) !!}
                    ...
                    <div class="content table-responsive table-full-width">
                        <table class="table table-striped">
                            ...
                            @elseif (Auth::user()->role == 'anggota')
                                @foreach (App\Wajib::whereUserId(Auth::user()->id)->orderBy('id', 'desc')->limit(20)->get() as $key => $row)
                                <tr id="tr{{$row->id}}">
                                    <td>{!! $key+1 !!}</td>
                                    <td>{!! $row->tanggal !!}</td>
                                    <td>{!! $row->user->no_anggota !!}</td>
                                ...
                                <tfoot>
                                    <tr style="font-weight: bolder;">
                                        <td colspan="4" class="text-center">TOTAL</td>
                                        <td>{!! number_format(App\Wajib::whereUserId(Auth::user()->id)->sum('masuk')) !!}</td>
                                        <td>{!! number_format(App\Wajib::whereUserId(Auth::user()->id)->sum('keluar')) !!}</td>
                                        @if (Auth::user()->role == 'admin' || Auth::user()->role == 'pengurus')
                                        <td></td>
                                        @else
                                        @endif
                        ...
                        @if (Auth::user()->role == 'admin' || Auth::user()->role == 'pengurus')
                            {!! $wajib->appends(['s' => Request::get('s')])->links() !!}
                        @endif
                    </div>
                </div>
            </div>
